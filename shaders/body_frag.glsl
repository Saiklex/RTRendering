...
uniform vec4 w1 = vec4(255.0/255.0, 176.0/255.0, 94.0/255.0, 1.0);
uniform vec4 w2 = vec4(255.0/255.0, 184.0/255.0, 108.0/255.0, 1.0);
...
void main() {
  ...
  // create a wood pattern with the predefined colours and the noise value
  vec4 woodColour = mix(w1, w2, v2);
  ...
  // mix wood colour with our detail matrices
  woodColour = mix(woodColour,pattern_just_noise,0.1);
  woodColour = mix(woodColour,pattern_splatter,0.3);

  // apply the environment map and shadows to the texture, then output
  woodColour = mix(woodColour,colour,0.05);
  FragColor = woodColour*shadeFactor*vec4(lightColor, 1.0);
}
